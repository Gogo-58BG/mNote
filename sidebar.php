<!-- Sidebar Holder -->
<nav id="sidebar">
    <div class="sidebar-header">
        <a href="https://www.motion-software.com" target="_blank"><h3>mNote</h3></a>
    </div>

    <ul class="list-unstyled components">
        <li>
            <a href="$note" data-toggle="modal" data-target="#myModal" class="note">><i class="fas fa-sticky-note"></i> New Note</a>
        </li>
        <li>
            <a href="#sortSubmenu" data-toggle="collapse" aria-expanded="false" class="dropdown-toggle"><i class="fas fa-sort"></i> Sort Notes</a>
            <ul class="collapse list-unstyled" id="sortSubmenu">
                <li>
                    <a href="#">by Done</a>
                </li>
                <li>
                    <a href="#">by Priority</a>
                </li>
                <li>
                    <a href="#">by Default</a>
                </li>
            </ul>
        </li>
        <li>
            <a href="#"><i class="fas fa-archive"></i> Arhived Notes</a>
        </li>
        <li>
            <a href="#"><i class="fas fa-trash"></i> Trash</a>
        </li>
        <li>
            <a href="#settingsSubmenu" data-toggle="collapse" aria-expanded="false" class="dropdown-toggle"><i class="fas fa-cogs"></i> Settings</a>
            <ul class="collapse list-unstyled" id="settingsSubmenu">
                <li>
                    <a href="#"><i class="fas fa-user-circle"></i> User Profile</a>
                </li>
                <li>
                    <a href="#"> Default Sort</a>
                </li>
            </ul>
        </li>
        <li>
            <a href="./php/logout.php"><i class="fas fa-sign-out-alt"></i> Logout</a>
        </li>
    </ul>

</nav>
    